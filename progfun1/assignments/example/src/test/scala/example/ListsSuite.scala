 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum of a few numbers") {
    assert(sum(List(1, 2, 0, 4)) === 7)
  }

  test("sum of a few numbers incl negatives") {
    assert(sum(List(1, -2, 0, 4)) === 3)
  }

  test("max of empty List throws error") {
  ...
  test("max of a few numbers") {
    assert(max(List(3, 7, 2, 4, 6)) === 7)
  }

  test("max of a few numbers incl negatives") {
    assert(max(List(3, -7, 2, 4, 6)) === 6)
  }
