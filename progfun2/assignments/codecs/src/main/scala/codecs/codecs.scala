...
trait EncoderInstances {
  ...
  implicit  val boolEncoder: Encoder[Boolean] =
    Encoder.fromFunction(e => Json.Bool(e))
...
trait DecoderInstances {
  ...
  implicit val boolDecoder: Decoder[Boolean] =
    Decoder.fromPartialFunction { case Json.Bool(e) => e }
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      case Json.Arr(arr) => {
        try {
          Some(arr map (x => decoder.decode(x).get))
        } catch {
  ...
  def field[A](name: String)(implicit decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction {
      case Json.Obj(fields) => fields.get(name) match {
        case None => None
...
trait PersonCodecs {
  ...
  implicit val personDecoder: Decoder[Person] =
    Decoder.field[String]("name")
      ...
      .transform[Person](x => Person(x._1, x._2))
