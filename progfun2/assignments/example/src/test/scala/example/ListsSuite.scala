 ...
 */
 class ListsSuite {
  ...
  @Test def `sum of an empty list (10pts)`: Unit = {
    assert(sum(List()) == 0)
  }
  ...
  @Test def `max of an empty list throws NoSuchElementException`: Unit = {
    try {
      max(List())
      Assert.fail("No exception has been thrown")
    } catch {
      case e: java.util.NoSuchElementException => ()
      case e: Throwable => Assert.fail("Exception other than NoSuchElementException thrown.")
    }
  }
