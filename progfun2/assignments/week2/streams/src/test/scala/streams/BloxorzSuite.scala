...
class BloxorzSuite {
  ...
  @Test def `neighborsWithHistory function level 1`: Unit =
    new Level1 {
      assertEquals(
  ...
  @Test def `newNeighborsOnly function level 1`: Unit =
    new Level1 {
      assertEquals(
        newNeighborsOnly(
          Set(
  ...
  trait LevelImpossible extends SolutionChecker {
    ...
    val optsolution: List[Move] = Nil
  }

  @Test def `solution length when no solution is possible (5pts)`: Unit =
    new LevelImpossible {
      assertEquals(optsolution.length, solution.length)
    }
