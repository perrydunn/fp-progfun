...
trait Solver extends GameDef {
  ...
  def done(b: Block): Boolean = {
    val g = goal
    b match {
      case Block(Pos(g.row, g.col), Pos(g.row, g.col)) => true
      case _ => false
    }
  }
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = for {
    legalNeighbor <- b.legalNeighbors.to(LazyList)
    (legalNeighborBlock, legalNeighborMove) = legalNeighbor
  } yield (legalNeighborBlock, legalNeighborMove :: history)
  ...
  def from(initial: LazyList[(Block, List[Move])],
           explored: Set[Block]): LazyList[(Block, List[Move])] = {
    val more = for {
      (thisBlock, theseMoves) <- initial
      next <- newNeighborsOnly(neighborsWithHistory(thisBlock, theseMoves), explored)
    } yield next
    if (!more.isEmpty) {
  ...
  lazy val solution: List[Move] = {
    val solution = pathsToGoal.headOption
    if (solution.isDefined) solution.get._2.reverse
    else Nil
  }
}
