...
object Server extends ServerModuleInterface {
  ...
  val eventParserFlow: Flow[ByteString, Event, NotUsed] =
    reframedFlow.map(x => Event.parse(x))
  ...
  val identityParserSink: Sink[ByteString, Future[Identity]] =
    reframedFlow.map(x => Identity.parse(x))
  ...
  val reintroduceOrdering: Flow[Event, Event, NotUsed] =
    Flow[Event].statefulMapConcat(() => {
      var expectedSequenceNr = 1
      var bufferedEvents: SortedSet[Event] = SortedSet[Event]()

      @tailrec
      def divideBufferedEventsIntoUnbufferedAndStillBuffered(
          sequenceNr: Int, unBufferedEvents: SortedSet[Event], bufferedEvents: SortedSet[Event]
      ): (Int, SortedSet[Event], SortedSet[Event]) = bufferedEvents.headOption match {
        case Some(event) if event.sequenceNr < sequenceNr =>
          divideBufferedEventsIntoUnbufferedAndStillBuffered(
            sequenceNr, unBufferedEvents + bufferedEvents.head, bufferedEvents.tail)
        case Some(event) if event.sequenceNr == sequenceNr =>
          divideBufferedEventsIntoUnbufferedAndStillBuffered(
            sequenceNr + 1, unBufferedEvents + bufferedEvents.head, bufferedEvents.tail)
        case _ => (sequenceNr, unBufferedEvents, bufferedEvents)
      }

      event =>
        if (event.sequenceNr == expectedSequenceNr) {
          val (nextExpectedSequenceNr, unbufferedEvents, stillBufferedEvents) =
            divideBufferedEventsIntoUnbufferedAndStillBuffered(
              expectedSequenceNr + 1, SortedSet(event), bufferedEvents)
          expectedSequenceNr = nextExpectedSequenceNr
          bufferedEvents = stillBufferedEvents
          unbufferedEvents
        } else {
  ...
  val followersFlow: Flow[Event, (Event, Followers), NotUsed] =
    Flow[Event].statefulMapConcat(() => {
      var followers: Followers = Map().withDefaultValue(Set.empty[Int])
  ...
  def isNotified(userId: Int)(eventAndFollowers: (Event, Followers)): Boolean =
    eventAndFollowers match {
      ...
      case (Event.Follow(_, _, toUserId), _) => toUserId == userId
      case (Event.PrivateMsg(_, _, toUserId), _) => toUserId == userId
      case (Event.StatusUpdate(_, fromUserId), followers) => followers.getOrElse(userId, Set()) contains fromUserId
      case _ => false
    }
...
//    clientIdPromise.future.map(id => actorSystem.log.info("Connected follower: {}", id.userId))
    ...
    val incoming: Sink[ByteString, NotUsed] =
      identityParserSink
        .mapMaterializedValue(futureOfIdentity => {
          clientIdPromise completeWith futureOfIdentity
          NotUsed
        })
