...
object Async extends AsyncInterface {
  ...
  def sequenceComputations[A, B](
    ...
    makeAsyncComputation1() flatMap {
      res1 => makeAsyncComputation2() map (res2 => (res1, res2))
    }
  ...
  def insist[A](makeAsyncComputation: () => Future[A], maxAttempts: Int): Future[A] = {
    val res = makeAsyncComputation()
    if (maxAttempts <= 1)
      res
    else
      res recoverWith { case _: Throwable => insist(makeAsyncComputation, maxAttempts - 1) }
  }
  ...
  def futurize(callbackBasedApi: CallbackBasedApi): FutureBasedApi = new FutureBasedApi {
    def computeIntAsync(): Future[Int] = {
      val p = Promise[Int]()
      callbackBasedApi.computeIntAsync(continuation = {
        case Success(v) => {
          p.trySuccess(v)
          ()
        }
