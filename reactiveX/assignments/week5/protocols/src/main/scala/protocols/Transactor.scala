...
object Transactor {
  ...
  private def inSession[T](rollbackValue: T, sessionTimeout: FiniteDuration, sessionRef: ActorRef[Session[T]]): Behavior[PrivateCommand[T]] =
    Behaviors.receivePartial[PrivateCommand[T]] {
      case (_, Committed(session, value)) =>
        if (session == sessionRef) idle(value, sessionTimeout)
  ...
  private def sessionHandler[T](currentValue: T, commit: ActorRef[Committed[T]], done: Set[Long]): Behavior[Session[T]] =
      Behaviors.receive[Session[T]] {
        case (_, Extract(func, replyTo: ActorRef[Any])) =>
          val newValue = func(currentValue)
          replyTo ! newValue
          Behaviors.same

        case (_, Modify(func, id, reply, replyTo: ActorRef[Any])) =>
          if (done contains id) {
            replyTo ! reply
            Behaviors.same
          } else {
